...
// when overexposing, chroma drops to 0
...
void expo_adjust_playback(int dir)
{
    ...
    for (int i = 0; i < buf_size; i += 2)
    {
        if (dir > 0)
        {
            uint8_t* luma = &current_buf[i+1];
            *luma = exp_inc[*luma];
            int8_t* chroma = &current_buf[i];
            *chroma = (int)(exp_inc[ABS(*chroma)*2]/2) * (int)SGN(*chroma) * (int)chroma_cor[*luma] / 256;
        }
        else
        {
            uint8_t* luma = &current_buf[i+1];
            *luma = exp_dec[*luma];
            int8_t* chroma = &current_buf[i];
            *chroma = (exp_dec[ABS(*chroma)*2]/2) * SGN(*chroma);
            //~ *chroma = (int)(exp_dec[ABS(*chroma)*2]/2) * SGN(*chroma) * chroma_cor[luma] / 256;
        }
    }
