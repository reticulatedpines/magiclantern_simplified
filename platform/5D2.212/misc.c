...
void display_shooting_info() // called from debug task
{
	...
	bmp_printf(fnt, X0+215, Y0+385, "%d%% ", GetBatteryLevel());

	if (lens_info.wb_mode == WB_KELVIN)
	{
		bmp_printf(fnt, X0+490, Y0+275, "%5dK", lens_info.kelvin);
	}
	if (lens_info.wbs_gm || lens_info.wbs_ba)
	{
		bg = bmp_getpixel(X0+15, Y0+460);
		fnt = FONT(FONT_MED, COLOR_FG_NONLV, bg);

		int ba = lens_info.wbs_ba;
		if (ba) bmp_printf(fnt, X0+350 + 2 * font_med.width, Y0+460, "%s%d", ba > 0 ? "A" : "B", ABS(ba));
		else    bmp_printf(fnt, X0+350 + 2 * font_med.width, Y0+460, "  ");

		int gm = lens_info.wbs_gm;
		if (gm) bmp_printf(fnt, X0+350, Y0+460, "%s%d", gm > 0 ? "G" : "M", ABS(gm));
		else    bmp_printf(fnt, X0+350, Y0+460, "  ");
	}
	...
	display_lcd_remote_icon(X0+555, Y0+460);
