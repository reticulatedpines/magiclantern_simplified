...
void display_shooting_info() // called from debug task
{
	...
	{
		bmp_printf(fnt, X0+360, Y0+279, "%5dK", lens_info.kelvin);
	}
	if (lens_info.wbs_gm || lens_info.wbs_ba)
	{
		...
		if (ba) bmp_printf(fnt, X0+320 + 2 * font_med.width, Y0+450, "%s%d", ba > 0 ? "A" : "B", ABS(ba));
		else    bmp_printf(fnt, X0+320 + 2 * font_med.width, Y0+450, "  ");

		int gm = lens_info.wbs_gm;
		if (gm) bmp_printf(fnt, X0+320, Y0+450, "%s%d", gm > 0 ? "G" : "M", ABS(gm));
		else    bmp_printf(fnt, X0+320, Y0+450, "  ");
	}
	...
	bmp_printf(fnt, X0+290, Y0+415, "%d%% ", GetBatteryLevel());
