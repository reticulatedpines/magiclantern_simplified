  ...
  while (l--) {
    if (*s == '"' || *s == '\\' || *s == '\n') {
      ...
      if (!isdigit(uchar(*(s+1))))
        snprintf(buff, 65536, "\\%d", (int)uchar(*s));
      else
        snprintf(buff, 65536, "\\%03d", (int)uchar(*s));
...
static int str_format (lua_State *L) {
  ...
  while (strfrmt < strfrmt_end) {
    if (*strfrmt != L_ESC)
      ...
      switch (*strfrmt++) {
        case 'c': {
          nb = snprintf(buff, 65536, form, (int)luaL_checkinteger(L, arg));
          break;
        }
        case 'd': case 'i':
...
static int str_pack (lua_State *L) {
  ...
  while (*fmt != '\0') {
    ...
    switch (opt) {
      ...
      case Kpadding: luaL_addchar(&b, LUA_PACKPADBYTE);  /* fall through */
