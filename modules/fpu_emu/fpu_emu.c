...
void (*fpu_emu_orig_undef_handler)(void) = 0;
...
static uint32_t fpu_emu_install()
{
    ...
    /* then patch handler */
    fpu_emu_orig_undef_handler = (void*)MEM(undef_handler_addr);
    MEM(undef_handler_addr) = (uint32_t)&fpu_emu_undef_handler;
    
    if(MEM(undef_handler_addr) != (uint32_t)&fpu_emu_undef_handler)
    {
        bmp_printf(FONT(FONT_MED, COLOR_RED, COLOR_BLACK), 0, 0, "fpu_emu: Failed to install trap handler");
        return 0;
    }
    
    bmp_printf(FONT(FONT_MED, COLOR_RED, COLOR_BLACK), 0, 0, "fpu_emu: installed");
    return 0;
}
...
/* accessor functions for NWFPE code */
